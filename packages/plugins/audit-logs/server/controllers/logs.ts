...
const controller = {
  async find(ctx: Core.KoaContext) {
    // RBAC: require read_audit_logs scope
    if (!ctx.state?.auth) {
      return ctx.unauthorized();
    }
    const scopes = ctx.state.auth?.credentials?.scopes || ctx.state.auth?.scope || [];
    const hasScope = Array.isArray(scopes) && scopes.includes('plugin::audit-logs.read_audit_logs');
    if (!hasScope) {
